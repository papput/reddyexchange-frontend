  ...
  return (
    <SitePageLayout>
      ...
      <main className="flex-1 container mx-auto px-4 py-16 max-w-3xl">
        {kicker && (
          <div className="text-xs uppercase tracking-widest text-primary font-bold mb-3">{kicker}</div>
        )}
        <h1 className="font-display text-4xl font-bold tracking-tight mb-6">{title}</h1>
        <div className="site-panel p-6 sm:p-8 prose prose-invert max-w-none text-secondary leading-relaxed [&_h2]:text-foreground [&_h2]:font-semibold [&_h2]:text-xl [&_h2]:mt-6 [&_h2]:mb-2 [&_strong]:text-foreground space-y-4">
          {children}
        </div>
        {showHelpStrip ? (
